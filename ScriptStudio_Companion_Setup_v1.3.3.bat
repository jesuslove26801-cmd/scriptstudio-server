...
curl.exe -L --output "D:\ScriptStudio\Companion\ScriptStudioCompanion.exe" "https://scriptstudio-web.pages.dev/ScriptStudioCompanion_v1.3.3.exe"
...
PowerShell -ExecutionPolicy Bypass -NoProfile -Command "Add-Type -AssemblyName System.Windows.Forms; [System.Windows.Forms.MessageBox]::Show('Companion v1.3.3 installed successfully!`n`nCompanion is running in the background.`nPC will auto-start Companion on next boot.`n`nClick the Grok Login button in the browser to sign in.', 'Setup Complete!', 'OK', [System.Windows.Forms.MessageBoxIcon]::Information)"
